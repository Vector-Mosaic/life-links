...
For a destructive operation, inspect the entire exact preview, revisions and effects and use the supported confirmation mechanism. In the page path the application owns the human dialog. A remote host must supply the required exact-preview interaction; a model boolean is not human approval, and a host that auto-approves has not thereby proved a human clicked. If the host cannot supply it, report the limit. Do not describe a required browser approval link as all-tabs-closed completion.
